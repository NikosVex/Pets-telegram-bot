...
async def add_pet_start(message: Message, state: FSMContext):
    ...
    await message.answer('Напишите имя питомца: ', reply_markup=kb.exit_button)
...
async def add_pet_name(message: Message, state: FSMContext):
    if len(message.text) <= 25:
        ...
        await message.answer('Напишите породу питомца: ', reply_markup=kb.exit_button)
    else:
...
async def add_pet_breed(message: Message, state: FSMContext):
    if len(message.text) <= 40:
        ...
        await message.answer('Напишите комментарий к питомцу: ', reply_markup=kb.exit_button)
    else:
        await message.answer('Порода питомца слишком длинная! Она должно быть не более 40 символов, '
                             'попробуйте ещё раз!', reply_markup=kb.exit_button)
...
async def add_pet_additionally(message: Message, state: FSMContext):
    if len(message.text) <= 200:
        ...
        await message.answer('Отправьте фотографию питомца: ', reply_markup=kb.exit_button)
    else:
...
async def add_pet_photo(message: Message, state: FSMContext):
    if message.photo:
        ...
        await message.answer('Отправьте, пожалуйста, ФОТОГРАФИЮ питомца', reply_markup=kb.exit_button)
